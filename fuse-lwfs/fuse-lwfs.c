...
// #include "authr/authr_clnt.h"
// #include "storage/ss_clnt.h"
#include "src/client/naming_client/naming_client.h"
#include "src/client/authr_client/authr_client.h"
#include "src/client/storage_client/storage_client.h"
// ... existing code ...
...
typedef struct lwfs_remote_pid lwfs_process_id;

typedef lwfs_nid lwfs_ptl_nid;
typedef lwfs_pid lwfs_ptl_pid;
...
int main(int argc, char *argv[])
{
	...
	// memset(&authr_svc, 0, sizeof(lwfs_service));
	...
	*/
	...
	rc = lwfs_get_service(args.authr, &authr_svc);
	if (rc != LWFS_OK) {
	...
	*/
	...
	rc = lwfs_get_service(args.naming, &naming_svc); 
	if (rc != LWFS_OK) {
	...
	*/
	...
	rc = lwfs_get_service(args.ss, &storage_svc);
	if (rc != LWFS_OK) {
	...
	// run_test(stdout, &naming_svc, storage_svc, 
	// 	  1, cid, &cap);
